...
-export([start/2, stop/1, produce/2, consume/1,init/1, handle_call/3, handle_cast/2]).
...
start(Callback, MaxSize) when is_atom(Callback) ->
    case erlang:function_exported(Callback, handle_produce, 1) andalso
        ...
        true ->
            gen_server:start(?MODULE, {Callback, MaxSize}, []);
        false ->
            {error, invalid_callback}
    end.

stop(Pid) -> gen_server:cast(Pid, stop).
produce(Pid, T) -> gen_server:call(Pid, {produce, T}).
consume(Pid) -> gen_server:call(Pid, consume).

init({Callback, MaxSize}) -> {ok, #{callback => Callback, max_size => MaxSize, queue => []}}.
